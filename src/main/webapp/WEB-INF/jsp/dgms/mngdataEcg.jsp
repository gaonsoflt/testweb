...
$(document).ready(function(){
	$('#mngdataEcg').parent().parent().addClass('active');
	$('#mngdataEcg').addClass('active');
});	
</script>
...
<!--                   <table id="example1" class="table table-bordered table-striped">
                ...
                $(document).ready(function () {
					...
					dataSourceDetail = new kendo.data.DataSource({
						transport: {
							read:  {
								url: crudServiceBaseUrl + "/selectDataEcgDetailInfoJsonp.do",
		            			dataType: "jsonp"
							},
							update: {
								url: crudServiceBaseUrl + "/updateDataEcgInfoJsonp.do",
								dataType: "jsonp"
							},
							destroy: {
								url: crudServiceBaseUrl + "/deleteDataEcgInfoJsonp.do",
								dataType: "jsonp"
							},
							create: {
								url: crudServiceBaseUrl + "/insertDataEcgInfoJsonp.do",
								dataType: "jsonp"
							},
...
/* 						aggregate: [
					...
					$("#gridDetail").kendoGrid({
						...
						excel: {
			                allPages: true,
			                fileName: "댁내용심전도측정내역.xlsx",
			            },
			            pdf: {
							allPages: true,
							fileName: "댁내용심전도측정내역.pdf",
							paperSize: "A4",
							landscape: true,
							scale: 0.75
						},      
...
/* 					$("#gridMaster").on("click", "table", function(e) {
    ...
    <script>
        ...
        function createChart(seq,pagenum,pagesize) {
        	$.ajax({
    			type: "get",
    			url: "<c:url value='/dgms/selectEcgInfoBySeq.do?ecg_seq="+seq+"'/>",//<c:url value='/appinf/selectMeasureScheduleInfo.do'/>
    			async: false, //동기 방식
    			success: function(data,status){
    	   	    		var calData_ = [];
						var allCalData_ = [];
